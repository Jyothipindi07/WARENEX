  ...
  return (
    <header className="h-16 border-b border-brand-border bg-brand-charcoal flex items-center justify-between px-6 select-none z-20 relative">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="flex items-center gap-4 border-l border-brand-border/60 pl-6">
          <button
            onClick={() => setShowNotifications(!showNotifications)}
            aria-label={`View notifications, ${unreadCount} unread`}
            aria-expanded={showNotifications}
            aria-controls="notifications-dropdown"
            className="relative p-2 text-gray-400 hover:text-brand-accent transition-colors focus:outline-none rounded-lg hover:bg-gray-800/50"
          >
